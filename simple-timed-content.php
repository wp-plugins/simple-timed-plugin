<?php
/*
Plugin Name: Artiss Timed Content
Plugin URI: http://www.artiss.co.uk/timed-content
...
define( 'timed_content_version', '1.2' );
...
function timed_content_plugin_meta( $links, $file ) {
	...
	if ( strpos( $file, 'simple-timed-content.php' ) !== false ) {
		$links = array_merge( $links, array( '<a href="http://www.artiss.co.uk/forum">' . __( 'Support', 'simple-content-reveal' ) . '</a>' ) );
		$links = array_merge( $links, array( '<a href="http://www.artiss.co.uk/donate">' . __( 'Donate', 'simple-content-reveal' ) . '</a>' ) );
	}
...
function timed_content_shortcode( $paras = '', $content = '' ) {
	...
	if ( calculate_timed_content( $ondate, $ontime, $onday, $offdate, $offtime, $offday ) ) {
		...
		$content = "<!-- Artiss Timed Content v" . timed_content_version . " | http://www.artiss.co.uk/timed-content -->\n" . $content . "<!-- End of Artiss Timed Content -->\n";
